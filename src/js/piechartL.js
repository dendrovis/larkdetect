...
var Dial = function(containerL) {
    this.containerL = containerL;
    this.size = this.containerL.dataset.size - 100; //control overall size
    this.strokeWidth = this.size / 8;
    this.radius = (this.size / 2) - (this.strokeWidth / 2);
    this.value = this.containerL.dataset.value;
    this.direction = this.containerL.dataset.arrow;
    this.svg;
    this.defs;
    this.slice;
    this.overlay;
...
Dial.prototype.create = function() {
    ...
    this.createText();
    this.createArrow();
    this.containerL.appendChild(this.svg);
};
...
Dial.prototype.createText = function() {
    var fontSize = this.size / 3.5; //size of font
    ...
    text.innerHTML = 0 + '<tspan font-size="' + tspanSize + '" dy="' + -tspanSize * 1.2 + '">sec</tspan>';
    this.svg.appendChild(text);
    this.text = text;
};

Dial.prototype.createArrow = function() {
    var arrowSize = this.size / 10;  // change arrow size
    var arrowYOffset, m;
...
var containersL = document.getElementsByClassName("chartL");
var dial = new Dial(containersL[0]);
dial.animateStart();